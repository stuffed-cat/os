...
pub fn init() {
    IDT.call_once(|| {
        ...
        unsafe {
            ...
            // For hardware builds, use a trampoline for GP faults so we can
            // inspect/modify the saved user context (e.g. skip a user-mode
        ...
        if let Some(table) = ProcessTable::global() {
                ...
                .and_then(|s| s.current_thread())
            {
                ...
                if let Some(proc) = table.lookup(pid) {
                    // Check if fault is in heap region (brk to brk+64MB) or mmap region (0x40000000 to 0x80000000)
                    let brk = proc.program_break();
...
// Hardware trampoline and handler to give low-level access to the saved registers
// for general protection faults. We use this to inspect the user instruction and
// optionally adjust RIP to skip a privileged HLT (0xF4) executed in user mode.
//
// Stack layout when handler is called:
// [high] ss, rsp, rflags, cs, rip, error_code, r15...rax (15 regs), ret_addr, align [low] <- actual rsp
// frame_ptr (rsi) = rsp + 15*8, points to error_code
#[cfg(feature = "hardware")]
#[unsafe(naked)]
extern "C" fn general_protection_trampoline() -> ! {
    naked_asm!(
        ...
        "push rbx",
        "push rax",
        "mov rdi, rsp",               // regs_ptr
        "lea rsi, [rsp + {frame_offset}]",  // frame_ptr points to error_code
        ...
        "iretq",
        frame_offset = const 15 * 8,
        handler = sym general_protection_trampoline_handler
    );
}

#[cfg(feature = "hardware")]
extern "C" fn general_protection_trampoline_handler(_regs_ptr: *const u8, frame_ptr: *const u8) {
    use core::ptr;

    // frame_ptr points to error_code on stack
    // Layout: error_code(0), rip(8), cs(16), rflags(24), rsp(32), ss(40)
    unsafe {
        let error_code = ptr::read(frame_ptr as *const u64);
        let rip_ptr = (frame_ptr as *const u64).add(1);
        let rip = ptr::read(rip_ptr);
        let rflags = ptr::read((frame_ptr as *const u64).add(3));
        let rsp = ptr::read((frame_ptr as *const u64).add(4));

        serial::write_fmt(format_args!(
            "\r\n=== USER GP FAULT (trampoline detail) ===\r\n"
        ));
        serial::write_fmt(format_args!("RIP: {:#x}\r\n", rip));
        serial::write_fmt(format_args!("RSP: {:#x}\r\n", rsp));
        serial::write_fmt(format_args!("RFLAGS: {:#x}\r\n", rflags));
        serial::write_fmt(format_args!("Error code: {:#x}\r\n", error_code));

        // Try to read instruction bytes at RIP
        if let Some(table) = ProcessTable::global() {
            if let Some(manager) = table.memory_manager() {
                                            ...
                                            if !l1_entry.flags().contains(PageTableFlags::PRESENT) {
                                                ...
                                                if let Some(phys_frame) = phys_frame {
                                                    ...
                                                    if instruction_byte == 0xf4 {
                                                        ...
                                                        serial::write_fmt(format_args!(
                                                            "Instruction at RIP is not HLT (got {:#x})\r\n", instruction_byte
                                                        ));
                                                    }
                                                }
                                            }
        ...
        // Default: record the fault and mark the process failed, matching the
        // behavior of the regular GP handler.
        crate::shell::record_user_fault(
            crate::shell::UserFaultKind::GeneralProtection,
            rip,
            0,
            error_code,
        );
