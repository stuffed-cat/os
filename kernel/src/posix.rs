...
impl<'a> PosixLayer<'a> {
    ...
    pub fn dispatch(&self, pid: Pid, syscall: SyscallId, args: &[u64]) -> Result<u64, Errno> {
        match syscall {
            ...
            SyscallId::ArchPrctl => {
                ...
                log::info!(
                    "arch_prctl: pid={} code={} addr={:#x} -> OK (stub)",
                    pid.as_u64(),
                    code,
                    addr
                );
            ...
            SyscallId::SetRobustList => {
                ...
                log::info!(
                    "set_robust_list: pid={} head={:#x} len={} -> OK (stub)",
                    pid.as_u64(),
                    head,
                    len
                );
            ...
            SyscallId::RtSigaction => {
                ...
                log::info!(
                    "rt_sigaction: pid={} signum={} -> OK (stub)",
                    pid.as_u64(),
                    signum
                );
                Ok(0)
            ...
            SyscallId::RtSigprocmask => {
                ...
                log::info!(
                    "rt_sigprocmask: pid={} how={} -> OK (stub)",
                    pid.as_u64(),
                    how
                );
                Ok(0)
            ...
            SyscallId::Prlimit64 => {
                ...
                log::info!(
                    "prlimit64: pid={} resource={} -> OK (stub)",
                    pid.as_u64(),
                    resource
                );
                Ok(0)
                ...
                if let Some((iov_base, mut iov_len)) = (|| unsafe {
                    ...
                    for offset in 0..iov_len {
                        if let Some(byte_phys) = (|| unsafe {
                            let l4_table_ptr = (phys_offset + root_frame.start_address().as_u64())
                                .as_u64()
                                as *const PageTable;
                            ...
                            unsafe {
                                let byte =
                                    ptr::read((phys_offset + byte_phys).as_u64() as *const u8);
                                if byte != 0 {
    ...
    unsafe fn translate_virt(
        virt: u64,
        l4_table_ptr: *const x86_64::structures::paging::PageTable,
        ...
        let l3_table_ptr =
            (phys_offset + l3_frame.start_address().as_u64()).as_u64() as *const PageTable;
        ...
        let l2_table_ptr =
            (phys_offset + l2_frame.start_address().as_u64()).as_u64() as *const PageTable;
        ...
        let l1_table_ptr =
            (phys_offset + l1_frame.start_address().as_u64()).as_u64() as *const PageTable;
